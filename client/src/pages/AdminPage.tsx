...
// Keep type import if you already have this type in your lib
import type { UserProfile } from "@/lib/googleSheetsApi";
...
const defaultMilestones: Milestone[] = [
  {
    ...
    description: "Complete your first full week of training",
    targetDate: "",
    completed: false,
    medal: "bronze",
  },
  {
    id: "2",
    title: "Weight Goal 25%",
    description: "Reach 25% of your weight loss goal",
    targetDate: "",
    completed: false,
    medal: "bronze",
  },
  {
    ...
    description: "Complete one month of consistent training",
    targetDate: "",
    completed: false,
    medal: "silver",
  },
  {
    id: "4",
    title: "Weight Goal 50%",
    description: "Reach halfway to your weight goal",
    targetDate: "",
    completed: false,
    medal: "silver",
  },
  {
    ...
    description: "Hit protein targets for 30 consecutive days",
    targetDate: "",
    completed: false,
    medal: "gold",
  },
...
// Set your admin creds here (match exactly what you want)
const ADMIN_USERNAME = "iamcap";
...
// If your backend returns raw rows+header, map them here.
// If backend already returns objects, you can remove this.
function mapRowsToObjects(header: string[], rows: any[][]) {
...
export default function AdminPage() {
  ...
  const [selectedUser, setSelectedUser] = useState<EnhancedUserProfile | null>(
    null,
  );
  const [isUserDialogOpen, setIsUserDialogOpen] = useState(false);
  const [isCreating, setIsCreating] = useState(false);

  const [userEditTab, setUserEditTab] = useState<
    "profile" | "milestones" | "notifications"
  >("profile");
  ...
  const isAdminCredsValid = (u: string, p: string) =>
    u.trim().toLowerCase() === ADMIN_USERNAME.toLowerCase() &&
    p.trim() === ADMIN_PASSWORD;
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!isAdminCredsValid(username, password)) {
  ...
  const enhanceUsers = (fetched: any[]): EnhancedUserProfile[] => {
    return fetched.map((user: any) => {
      // Support both user_id and userid naming
      const uid = user.user_id || user.userid || user.id || user.email || "user";

      const extUser = user as any;
      const planStart =
        extUser.plan_start_date || extUser.planstartdate || new Date().toISOString().split("T")[0];
      const planEnd = extUser.plan_end_date || extUser.planenddate || "";
  ...
  const loadUsers = async () => {
    ...
    const res = await postJSON<{
      ...
      header?: string[];
      rows?: any[][];
    }>("/api/admin/users/list", { username, password });

    if (!res.ok) {
      setUsers([]);
      toast({
        title: "Error",
        description: res.error || "Failed to load users",
        variant: "destructive",
      });
      setLoading(false);
      return;
    ...
    // If backend returns {users:[...]} use directly
    let fetchedUsers: any[] = [];
    if (Array.isArray(res.users)) {
      fetchedUsers = res.users;
    } else if (Array.isArray(res.header) && Array.isArray(res.rows)) {
      fetchedUsers = mapRowsToObjects(res.header, res.rows);
    }

    setUsers(enhanceUsers(fetchedUsers));
    setLoading(false);
  };
  ...
  const handleCreateUser = async () => {
    ...
    const res = await postJSON<{ generated_password?: string }>(
      "/api/admin/users/create",
      { username, password, user: newUser },
    );
    ...
    toast({
      title: "User Created",
      description: `User created successfully. Generated password: ${res.generated_password || "Check email"}`,
    });
  ...
  const handleUpdateUser = async () => {
    ...
    const userId = selectedUser.user_id || (selectedUser as any).userid;
    const res = await postJSON<{}>("/api/admin/users/update", {
      username,
      password,
      userId,
      user: selectedUser,
  ...
  const toggleMilestone = (milestoneId: string) => {
    ...
    const updatedMilestones = selectedUser.milestones?.map((m) => {
      ...
      if (completed && m.medal && selectedUser.medals) {
        ...
        // Update both medals and milestones in one state update
        return { ...m, completed, __newMedals: newMedals } as any;
      }

      return { ...m, completed };
    });

    if (!updatedMilestones) return;

    // Pull __newMedals if present
    const anyWithMedals = updatedMilestones.find((m: any) => m.__newMedals);
    const newMedals = anyWithMedals?.__newMedals;

    const cleaned = updatedMilestones.map((m: any) => {
      const { __newMedals, ...rest } = m;
      return rest;
    });
    ...
    setSelectedUser({
      ...selectedUser,
      milestones: cleaned,
      medals: newMedals || selectedUser.medals,
    });
  };
  ...
  const sendNotification = () => {
    ...
    const notification: UserNotification = {
      ...
      userId: selectedUser.user_id || (selectedUser as any).userid || "",
    ...
    toast({
      ...
      description: "Notification will be sent to the user.",
    });
  };
          ...
          <Card className="w-full max-w-md p-8 bg-card/40 border-white/5 backdrop-blur-xl">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Tip: username is case-insensitive in this page.
              </p>
            </form>
          </Card>
        </div>
      ...
      <div className="space-y-8">
        ...
        {activeTab === "overview" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <Card
              className="bg-card/40 border-white/5 p-6 hover:border-red-500/50 transition-all cursor-pointer group"
              onClick={() => setActiveTab("content")}
            >
              ...
              <p className="text-muted-foreground text-sm mt-2">
                Add new exercises to the global repository via Google Sheets.
              </p>
            </Card>
            ...
            >
              ...
              <p className="text-muted-foreground text-sm mt-2">
                Push new training blocks and weekly schedules.
              </p>
            </Card>
            ...
            >
              ...
              <p className="text-muted-foreground text-sm mt-2">
                Create, edit, and manage member accounts.
              </p>
            </Card>

            <Card
              className="bg-card/40 border-white/5 p-6 hover:border-yellow-500/50 transition-all cursor-pointer group"
              onClick={() => setActiveTab("users")}
            >
              ...
              <p className="text-muted-foreground text-sm mt-2">
                Track milestones and award achievement medals.
              </p>
            </Card>

            <Card
              className="bg-card/40 border-white/5 p-6 hover:border-blue-500/50 transition-all cursor-pointer group"
              onClick={() => setActiveTab("users")}
            >
              ...
              <p className="text-muted-foreground text-sm mt-2">
                Motivate members with personalized messages.
              </p>
            </Card>

            <Card
              className="bg-card/40 border-white/5 p-6 hover:border-green-500/50 transition-all cursor-pointer group"
              onClick={() => setActiveTab("content")}
            >
              ...
              <p className="text-muted-foreground text-sm mt-2">
                Add and edit nutrition recipes with macros.
              </p>
            </Card>
          </div>
        )}
              ...
              <Card className="bg-card/40 border-white/5 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNewUser({
                          ...newUser,
                          current_weight: Number(e.target.value),
                        })
                      }
                      className="bg-black/20 border-white/10"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      type="number"
                      value={newUser.goal_weight || ""}
                      onChange={(e) =>
                        setNewUser({
                          ...newUser,
                          goal_weight: Number(e.target.value),
                        })
                      }
                      className="bg-black/20 border-white/10"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={newUser.membership_type as any}
                      onValueChange={(v) =>
                        setNewUser({ ...newUser, membership_type: v })
                      }
                    >
                      <SelectTrigger className="bg-black/20 border-white/10">
                        <SelectValue />
                    ...
                    {users.length === 0 && (
                      <tr>
                        ...
                        >
                          {loading
                            ? "Loading users..."
                            : "No users found. Users will appear here when loaded from Google Sheets."}
                        </td>
                      </tr>
                    )}

                    {users.map((user) => {
                      const uid = user.user_id || (user as any).userid || user.email;
                      return (
                        <React.Fragment key={uid}>
                          ...
                          >
                            ...
                            <td className="p-4">
                              <div className="w-32">
                                <Progress
                                  value={calculatePlanProgress(user)}
                                  className="h-2"
                                />
                                <p className="text-xs text-muted-foreground mt-1">
                                  {calculatePlanProgress(user)}% complete
                                </p>
                              </div>
                            </td>
                            ...
                            <td className="p-4">
                              <span
                                className={`font-bold ${
                                  getDaysRemaining(user.plan_end_date || "") < 7
                                    ? "text-red-400"
                                    : "text-white"
                                }`}
                              >
                                {getDaysRemaining(user.plan_end_date || "")}{" "}
                                days
                              </span>
                            </td>
                            ...
                            <td className="p-4">
                              <div className="flex gap-1 items-center">
                                <span className="text-sm">
                                  {MEDAL_ICONS.bronze} {user.medals?.bronze || 0}
                                </span>
                                <span className="text-sm">
                                  {MEDAL_ICONS.silver} {user.medals?.silver || 0}
                                </span>
                                <span className="text-sm">
                                  {MEDAL_ICONS.gold} {user.medals?.gold || 0}
                                </span>
                                <span className="text-sm">
                                  {MEDAL_ICONS.platinum}{" "}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                  <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-white">
                                      Start: {user.plan_start_date || "Not set"}
                                    </p>
                                    <p className="text-sm text-white">
                                      End: {user.plan_end_date || "Not set"}
                                    </p>
                                    <p className="text-sm text-white">
                                      Assigned: {(user as any).plan_assigned || "None"}
                                    </p>
                                  </div>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-white">
                                      Calories: {(user as any).calorie_target} kcal
                                    </p>
                                    <p className="text-sm text-white">
                                      Protein: {(user as any).protein_target}g
                                    </p>
                                    <p className="text-sm text-white">
                                      Goal Weight: {(user as any).goal_weight}kg
                                    </p>
                                  </div>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-white">
                                      {user.milestones?.filter((m) => m.completed)
                                        .length || 0}{" "}
                                      / {user.milestones?.length || 0} completed
                                    </p>
                                    <p className="text-sm text-white">
                                      Total Medals: {getTotalMedals(user)}
                                    </p>
                                  </div>
                                </div>
                              </td>
                ...
                <Card className="bg-card/40 border-white/5 p-8">
                  ...
                  <div className="grid gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        placeholder={"1. Blend yogurt with protein powder\n2. Pour into bowl..."}
                        className="bg-black/20 border-white/10 min-h-[100px]"
                      />
                    </div>
                  ...
                  <div className="grid gap-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          placeholder="e.g., 12-Week Fat Loss"
                          className="bg-black/20 border-white/10"
                        />
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        placeholder="A comprehensive plan designed to..."
                        className="bg-black/20 border-white/10"
                      />
                    </div>
                  ...
                  <div className="grid gap-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          placeholder="e.g., 20 Ways to Hit 100g Protein"
                          className="bg-black/20 border-white/10"
                        />
                      </div>
        ...
        <Dialog open={isUserDialogOpen} onOpenChange={setIsUserDialogOpen}>
          <DialogContent className="max-w-3xl bg-card border-white/10 text-white max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Update user profile, milestones, and send notifications.
              </DialogDescription>
            </DialogHeader>

            <Tabs
              value={userEditTab}
              onValueChange={(v) => setUserEditTab(v as any)}
              className="mt-4"
            >
                ...
                {selectedUser && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                    <div className="space-y-2">
                      <Label>Name</Label>
                      <Input
                        value={selectedUser.name as any}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                    ...
                    <div className="space-y-2">
                      <Label>Email</Label>
                      <Input
                        value={selectedUser.email as any}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="date"
                        value={selectedUser.plan_start_date || ""}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            plan_start_date: e.target.value,
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="date"
                        value={selectedUser.plan_end_date || ""}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            plan_end_date: e.target.value,
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="number"
                        value={(selectedUser as any).age || 0}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            age: Number(e.target.value) as any,
                          })
                        }
                        className="bg-black/20 border-white/10"
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="number"
                        value={(selectedUser as any).height_cm || 0}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            height_cm: Number(e.target.value) as any,
                          })
                        }
                        className="bg-black/20 border-white/10"
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="number"
                        value={(selectedUser as any).current_weight || 0}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            current_weight: Number(e.target.value) as any,
                          })
                        }
                        className="bg-black/20 border-white/10"
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="number"
                        value={(selectedUser as any).goal_weight || 0}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            goal_weight: Number(e.target.value) as any,
                          })
                        }
                        className="bg-black/20 border-white/10"
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="number"
                        value={(selectedUser as any).calorie_target || 0}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            calorie_target: Number(e.target.value) as any,
                          })
                        }
                        className="bg-black/20 border-white/10"
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        type="number"
                        value={(selectedUser as any).protein_target || 0}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            protein_target: Number(e.target.value) as any,
                          })
                        }
                        className="bg-black/20 border-white/10"
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        value={(selectedUser as any).plan_assigned || ""}
                        onChange={(e) =>
                          setSelectedUser({
                            ...selectedUser,
                            plan_assigned: e.target.value as any,
                          })
                        }
                        className="bg-black/20 border-white/10"
                      />
                    ...
                    <div className="space-y-2">
                      <Label>Membership Type</Label>
                      <Select
                        value={(selectedUser as any).membership_type}
                        onValueChange={(v) =>
                          setSelectedUser({
                            ...selectedUser,
                            membership_type: v as any,
                          })
                        }
                      >
                        <SelectTrigger className="bg-black/20 border-white/10">
                  ...
                  <div className="space-y-4 py-4">
                    <div className="flex items-center gap-4 p-4 bg-black/20 rounded-lg">
                      <div className="flex gap-3">
                        <div
                          className={`px-3 py-2 rounded-lg ${MEDAL_COLORS.bronze} border`}
                        >
                          <p className="text-2xl text-center">
                            {MEDAL_ICONS.bronze}
                          </p>
                          <p className="text-sm font-bold">
                            {selectedUser.medals?.bronze || 0}
                          </p>
                        </div>

                        <div
                          className={`px-3 py-2 rounded-lg ${MEDAL_COLORS.silver} border`}
                        >
                          <p className="text-2xl text-center">
                            {MEDAL_ICONS.silver}
                          </p>
                          <p className="text-sm font-bold">
                            {selectedUser.medals?.silver || 0}
                          </p>
                        </div>

                        <div
                          className={`px-3 py-2 rounded-lg ${MEDAL_COLORS.gold} border`}
                        >
                          <p className="text-2xl text-center">
                            {MEDAL_ICONS.gold}
                          </p>
                          <p className="text-sm font-bold">
                            {selectedUser.medals?.gold || 0}
                          </p>
                        </div>

                        <div
                          className={`px-3 py-2 rounded-lg ${MEDAL_COLORS.platinum} border`}
                        >
                          <p className="text-2xl text-center">
                            {MEDAL_ICONS.platinum}
                          </p>
                          <p className="text-sm font-bold">
                            {selectedUser.medals?.platinum || 0}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      {selectedUser.milestones?.map((milestone) => (
                        ...
                        >
                          ...
                          {milestone.medal && (
                            <div
                              className={`px-2 py-1 rounded text-sm ${MEDAL_COLORS[milestone.medal]} border`}
                            >
