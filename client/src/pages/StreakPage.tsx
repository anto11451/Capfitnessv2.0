...
export default function StreakPage() {
  ...
  // OPTIONAL: sync from sheet in background
  if (!user?.id) return;
  syncFromSheet();
}, [user?.id]);
useEffect(() => {
...
setStreakData(prev => {
  ...
  selectedDays.forEach(date => {
    const idx = newHistory.findIndex(h => h.date === date);
    const log = {
      date,
      workoutDone: true,
      dietDone: true,
    ...
    if (idx >= 0) newHistory[idx] = log;
    else newHistory.push(log);
  });

  const { current, longest } = calculateStreak(newHistory);
  return { ...prev, history: newHistory, currentStreak: current, longestStreak: longest };
});

// THEN sync to API (fire & forget)
Promise.all([...selectedDays].map(d =>
  updateStreak(user.id, d, {
    workout_done: true,
    diet_done: true,
    rest_day: false
  })
      ...
      <Dialog open={isBulkDialogOpen} onOpenChange={setIsBulkDialogOpen}>
        <DialogContent className="bg-card border-white/10 text-white">
          ...
          <div className="space-y-3 py-4">
            ...
            >
                {loading ? "Updating..." : `Update ${selectedDays.size} Days`}
</Button>
